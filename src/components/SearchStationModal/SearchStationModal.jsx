...
const SearchStationModal = ({ isOpen, toggleModal, favoriteStations, toggleFavorite }) => {
  ...
  const { data: stationData, load: stationLoad, error: stationError, fetchData: stationFetchData } = useFetchData("https://devse.gonetis.com/api/station");
  ...
  const handleSearch = async (stationName) => {
    setIsLoading(true);
    try {
      const endpoint = `https://bbbserver.devse.kr/api/station?name=${stationName}`;
